...
jQuery.fn.dupa = function (x) {
    ...
    var q = x.images && (new dupaPreloader(this, x));
    var i = a.find("li");
    var B = i.length;
    ...
    if (x.preventCopy && !/iPhone/.test(navigator.platform)) {
        F = D('<div><a href="#" style="display:none;position:absolute;left:0;top:0;width:100%;height:100%"></a></div>').css({
            position: "absolute",
            left: 0,
            top: 0,
            width: "100%",
        ...
        if (l.length) {
            ...
            if (Y.length) {
                ...
                function ac(aj) {
                    ...
                    var ai = Y.get(aj);
                    var ah = {
                        left: -ai.offsetLeft + (D(ai).outerWidth(true) - D(ai).outerWidth()) / 2
                    };
                    if (c < 0) {
                        Z.css(ak);
                        f.css(ah)
    ...
    function G(c) {
        D("A", L).each(function (aa) {
            if (aa == c) {
                ...
                if (!E) {
                    var f = L.find(">div"),
                        Z = Y.position() || {}, ab = f.position() || {};
                    f.stop(true).animate({
                        left: -Math.max(Math.min(Z.left, -ab.left), Z.left + Y.width() - L.width()),
    ...
    function N(ad, ag) {
        ...
        ad.show();
        var ac = {
            width: v(ad.get(0), 1, 1),
            height: s(ad.get(0), 1, 1),
            "float": ad.css("float"),
            overflow: "hidden",
            opacity: 0
        };
